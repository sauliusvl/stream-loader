...
/**
  ...
  */
class ExternalOffsetVerticaBatchStorage(
  ...
  /**
    ...
    */
  override protected def commitBatchWithOffsets(bb: FormattedRecordBatch[Unit, ExternalVerticaRowBatch]): Unit = {
    val batch = bb.partitionBatches(())
    Using.resource(dbDataSource.getConnection) { connection =>
      connection.setAutoCommit(false)
      val copyQuery = batch.copyStatement(table)
      try {
        val inserts = bb.recordRanges.map { range =>
          ...
          batchInsertStatement.setLong(1, batch.id)
...
object ExternalOffsetVerticaBatchStorage {
  ...
  case class Builder(
    ...
    /**
      ...
      * Used for querying the offset during initialization to look up committed stream positions.
      */
    ...
    def build(): ExternalOffsetVerticaBatchStorage = {
      ...
      new ExternalOffsetVerticaBatchStorage(
        _dbDataSource,
        _table,
        _offsetTable,
        _fileIdColumnName,
