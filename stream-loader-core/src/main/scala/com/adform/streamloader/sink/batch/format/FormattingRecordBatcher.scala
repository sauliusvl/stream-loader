package com.adform.streamloader.sink.batch.format

import com.adform.streamloader.model.StreamRecord
import com.adform.streamloader.sink.batch.{BatchBuilder, RecordBatcher}
...
object FormattingRecordBatcher {
  ...
  class UnpartitionedBuilder[R](_formatter: RecordFormatter[R]) {
    ...
    def batchBuilder[B](builder: () => BatchBuilder[R, B]): Builder[R, Unit, B] =
      new Builder(_formatter, (raw: StreamRecord, formatted: R) => (), (_: Unit) => builder())
  }

  class PartitionedBuilder[R, P](_formatter: RecordFormatter[R], _partitioner: RecordPartitioner[R, P]) {
    def batchBuilder[B](builder: P => BatchBuilder[R, B]): Builder[R, P, B] =
      new Builder(_formatter, _partitioner, builder)
  }
